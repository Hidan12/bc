...
const SelectMinimalista = ({
  ...
  return (
    <div className={`w-full flex flex-col ${backGraundInput} ${border != "border-b-2" ? `${border} ${borderCol}` : ""}`} >
        <span className={`${labelColor} text-[12px] pl-2 pt-1`}>{label}</span>
        <div
        ref={containerRef}
        tabIndex={0}
        onClick={toggleDropdown}
        onKeyDown={handleKeyDown}
        className={`w-full flex flex-col relative cursor-pointer outline-none ${border == "border-b-2" ? `p-2 ${border}`: "p-2"} ${error ? borderColError : borderCol}`}
        >
        <div className="w-full flex justify-between items-center">
            <span>{selectedItem}</span>
            <div className={`${colorSvg} transform transition-transform duration-300 ${!isOpen ? "rotate-180": ""}`}>
        ...
        {isOpen && (
            <div
            className={`p-3 z-20 absolute bg-white w-full left-0 top-10 flex flex-col overflow-y-auto ${backGraundInput} ${
